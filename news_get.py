...
proxy_id = 0   #全局变量，如果当前ip无法用，就更换到下一个取
...
def extract_by_newspaper(url1, url3,filename,search_word_i,ip_pool_all,up_date,gov_name_i, page_num = 501):
    '''
    ...
    for url_2 in range(1, page_num):
        ...
        for item in urls:
            ...
            try:
                ...
                print(e)
                while(True):
                    ip_proxy_now = ip_proxy_now+1   # 更换ip池，+1
                    try:
                        paper = Article(url_item, language='zh', memoize_articles=False, config=config)
                        break
                    except:
                        ip_proxy_now = ip_proxy_now + 1
                        pass
                pass
            # print(type(paper.publish_date))

            if (isinstance(paper.publish_date, datetime.datetime) == True) and paper.publish_date >= d.strptime(up_date, "%Y-%m-%d"):     # 要求官员的上任日期xxx
                # 将数据存入进去
...
def get_page_num(url):
    ...
    html_data = parsel.Selector(data)
    paser_list = html_data.xpath('//*[@id="results"]/span/text()').extract_first()
    ## 已经找到了需要的关键信息
...
if __name__ == '__main__':
    ...
    # for class_gov in [3]:
        ...
        for i in range(len(words)):
            ...
            # 构建搜索网页，然后进行搜索界面
            url_1 = 'http://search.youth.cn/cse/search?q=' + words[i] + '&p='
            # url_2 = 1  #这个是一个数字
            url_3 = '&s=15107678543080134641&nsid=&entry=1'

            page_num = get_page_num(url_1+str(1)+url_3)  # 搜索的页面数目--构建搜索页面数目
            # print(page_num)
            if page_num == None or page_num == 0:
                ...
                # 挖掘新闻信息并且存入excel中
                extract_by_newspaper(url_1,url_3,filename,i,ip_pool_all,up_date,words[i][0:3],page_num)
